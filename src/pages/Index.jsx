        ...
        <CardContent className="bg-white p-4">
          ...
          >
            <div className="mt-4">
              ...
              <ul className="list-disc list-inside text-sm text-blue-600 mb-4">
                {animal.facts.map((fact, index) => (
                  <li key={index}>{fact}</li>
                ))}
              </ul>
              <h3 className="font-bold text-blue-700 mb-2">Key Traits:</h3>
          ...
          <div className="flex justify-between items-center mt-4">
            ...
            >
              <Heart className={`h-5 w-5 ${likes > 0 ? 'text-red-500 fill-red-500' : 'text-gray-500'}`} />
              <span>{likes}</span>
            </Button>
          </div>
        </CardContent>
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-blue-50 to-white py-12 px-4 sm:px-6 lg:px-8">
      <div className="max-w-6xl mx-auto">
        <motion.h1 
          className="text-5xl font-bold text-center mb-8 text-blue-800 flex items-center justify-center"
          initial={{ opacity: 0, y: -50 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ duration: 0.5 }}
        >
